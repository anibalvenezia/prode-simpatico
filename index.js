    ...
    .then((data) => {
      data.forEach(d => {
        ...
        if(d.status==="completed"){
            ...
            if(resultado_partido===benit[d.id - 1].resultado || resultado_partido===benit[d.id - 1].resultado2)
                players.forEach(p => {
                    if(p.name==='benit') p.points++
                });
            if(resultado_partido===tincho[d.id - 1].resultado || resultado_partido===tincho[d.id - 1].resultado2)
                players.forEach(p => {
                    if(p.name==='tincho') p.points++
            });
            if(resultado_partido===negro[d.id - 1].resultado || resultado_partido===negro[d.id - 1].resultado2)
                players.forEach(p => {
                    if(p.name==='negro') p.points++
            });
            if(resultado_partido===gringo[d.id - 1].resultado || resultado_partido===gringo[d.id - 1].resultado2)
                players.forEach(p => {
                    if(p.name==='gringo') p.points++
            });
            if(resultado_partido===player[d.id - 1].resultado || resultado_partido===player[d.id - 1].resultado2)
                players.forEach(p => {
        ...
        //playersPoints.forEach(f => console.log(f));
        let tbody = $('#body');
        let position=1;
